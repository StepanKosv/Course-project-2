...
public class InterfaceFX extends Application {
	...
	public void start(Stage stage) throws Exception {
        ...
        FXMLLoader loader = new FXMLLoader(getClass().getResource("/com/stepankosvin/app_interface/ScopeSelect.fxml"));
        
        // 2. Загружаем иерархию компонентов
        Parent root = loader.load();
        
        // 3. Получаем ссылку на контроллер
        ScopeSelectController controller = loader.getController();
        
        //Настройка бд
        EntityManagerFactory emf = Persistence.createEntityManagerFactory("reddit-collector");
        DBFasade fasade=new DBFasade(emf);
        controller.initData(fasade);
	...
	private void testScopeSelect(Stage stage) throws Exception {
		
		// 1. Создаем объект загрузчика
        FXMLLoader loader = new FXMLLoader(getClass().getResource("ScopeSelect.fxml"));
	...
	private void testCollectVigets(Stage stage) throws Exception {
		EntityManagerFactory emf = Persistence.createEntityManagerFactory("reddit-collector");
		IScopeHolder holder = new ScopeHolder("test result writer ver 2", -1L, emf);
        ...
        controller.initData(holder, List.of(
        		"https://www.reddit.com/r/mathmemes/comments/1sgzvxx/the_1phone/.json"));
        javafx.application.Platform.runLater(()->{
	...
	private void testGraphView(Stage stage) throws Exception {
		EntityManagerFactory emf = Persistence.createEntityManagerFactory("reddit-collector");
		IScopeHolder holder = new ScopeHolder("test display scope", -1L, emf);
		var db=holder.getDbFasade();

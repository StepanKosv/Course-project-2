...
public class CollectSettingsController  {
    ...
    @FXML public Spinner<Integer> pagesSpinner;
    @FXML public ListView<String> initialPagesList;
    @FXML private TextField pageInputField;
    @FXML private Button closeButton;
    @FXML private Button startButton;
    public IScopeHolder dataHolder;
 ...
 // Этот метод мы вызовем ВРУЧНУЮ для передачи данных
    public void initData(IScopeHolder holder, List<String> defaultPages) {
    ...
    private void handleStart() {
    	...
    	try {
    		if(processStatus==null&&
    				initialPagesList.getItems().size()>0&&
    				pagesSpinner.getValue()>0) {
    			...
    			controller.setParentAndInit(this);
